...
def main(cfg: FairseqConfig) -> None:
    ...
    '''
    #normalize the fisher matrix to have a mean value of 1.0
    for key in fisher_matrix.keys():
        fisher_matrix[key] = fisher_matrix[key] / fisher_matrix[key].mean()
    '''

    fisher_save = open(os.path.join(cfg.checkpoint.save_dir, 'fisher_matrix.pt'), 'wb')
    torch.save(fisher_matrix, fisher_save)
    
    train_meter.stop()
    logger.info("done EWC computation in {:.1f} seconds".format(train_meter.sum))
...
def fisher(
    ...
    """Train the model for one epoch and conpute fisher information matrix."""
    ...
    fisher_matrix = {}
    fisher_matrix['task_id'] = torch.tensor(1)
    extra_kwargs = {}
    ...
    if os.path.exists(os.path.join(cfg.checkpoint.save_dir, 'fisher_matrix.pt')):
        old_fisher_matrix = torch.load(os.path.join(cfg.checkpoint.save_dir, 'fisher_matrix.pt'))
        task_id = old_fisher_matrix['task_id']
        for n, p in fisher_matrix.items():
            fisher_matrix[n] = (fisher_matrix[n] + old_fisher_matrix[n]*(task_id)) / (task_id + 1)
        fisher_matrix['task_id'] = task_id + 1
